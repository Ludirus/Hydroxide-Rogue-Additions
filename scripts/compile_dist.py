...
TARGETS = {
    ...
    "hydrogen.lua": {
        "source": ROOT / "Hydrogen" / "hydrogen.lua",
...
def build_encrypted_script(source_path: Path, repo_base: str, entrypoint: str, chunk_size: int, quiet: bool = False) -> tuple[str, int, int, int]:
    source_prefix = (
        "if getgenv then "
...
def compile_target(source_path: Path, output_path: Path, repo_base: str, entrypoint: str, chunk_size: int, quiet: bool = False) -> None:
    script, seed, carry, plaintext_size = build_encrypted_script(source_path, repo_base, entrypoint, chunk_size, quiet)
...
def main() -> None:
    ...
    for output_name, target in TARGETS.items():
        ...
        compile_target(
            ...
            DIST / output_name,
            repo_base,
            entrypoints[output_name],
            args.chunk_size,
            bool(target.get("quiet", False)),
        )
